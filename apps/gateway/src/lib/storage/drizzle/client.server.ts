 ...
 * file-backed PGlite at `FICTA_GATEWAY_DATA_DIR` (default `.data/pglite`). The zero-config PGlite default is
...
const KEY = Symbol.for("ficta.gateway.db");
...
function create(): Handle {
  ...
  const dataDir = process.env.FICTA_GATEWAY_DATA_DIR ?? ".data/pglite";
