...
| `TYPESAFE_API_KEY`                                                                    | Server-only TypeSafe API key; makes the optional second opinion available in Admin settings (advisory, fail-open); see the PII threat model before setting it                                 | -                                     |
| `FICTA_GATEWAY_SECOND_OPINION`                                                        | Optional `on` or `off` pin for the second opinion; the Admin settings toggle becomes read-only. Unset lets admins decide (default off)                                                        | unset                                 |
| `FICTA_GATEWAY_SECOND_OPINION_MODEL`, `FICTA_GATEWAY_SECOND_OPINION_TIMEOUT_MS`       | Pinned TypeSafe model and per-request timeout for the second opinion                                                                                                                          | `jev-1.13.0`, `4000`                  |
