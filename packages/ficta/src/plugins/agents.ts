...
function claudeMachineReadableCommand(args: readonly string[]): boolean {
    ...
    const format =
      arg === "--output-format"
        ...
        : arg?.startsWith("--output-format=")
          ? arg.slice("--output-format=".length).toLowerCase()
          : undefined;
    if (format === "json" || format === "stream-json") return true;
  }
  return false;
}
