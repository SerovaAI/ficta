...
export interface StartupBannerOptions {
  ...
  /** Active surrogate token style. Defaults to "opaque" when omitted. */
...
export function shouldPrintStartupDiagnostics(opts: {
  verbose: boolean;
  stderrIsTTY?: boolean;
  ...
  // spawn, and machine-readable subprocess calls often treat stderr as part of an error payload.
  // Keep those clean by default, while preserving diagnostics for other logs/pipes and explicit
  // debug runs.
...
export function renderStartupBanner(opts: StartupBannerOptions): string {
  ...
  const secretShapesActive = opts.discoveries.some((d) => d.id === "secret-shapes/detector" && d.status === "active");
  const lines = [
    ...
    `   secret shapes: ${formatSecretShapesStatus(secretShapesActive)}`,
    `   pii: ${formatPiiStatus(piiActive, opts.piiFailClosed)}`,
    `   surrogates: ${formatSurrogateStyle(opts.surrogateStyle ?? "opaque")}`,
  ];
...
function formatSecretShapesStatus(active: boolean): string {
  return active ? "on (known token shapes, best-effort)" : "off";
}

/** Surrogate token style: opaque `FICTA_<hex>` (default) vs typed `FICTA_<TYPE>_<hex>`. */
function formatSurrogateStyle(style: "opaque" | "typed"): string {
  return style === "typed" ? "typed (FICTA_<TYPE>_… tokens)" : "opaque (FICTA_… tokens)";
}
