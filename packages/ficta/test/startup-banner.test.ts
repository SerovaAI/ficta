...
describe("startup banner", () => {
  it("renders a compact default summary", () => {
    ...
    expect(rendered).toBe(
      "🔒 ficta ready — 47 protected values (48 loaded before dedupe)\n" +
        ...
        "   secret shapes: off\n" +
        "   pii: off\n" +
        "   surrogates: opaque (FICTA_… tokens)\n",
    );
  ...
  it("shows the surrogate token style, defaulting to opaque", () => {
    const base = { protectedValues: 0, agentCommand: "claude", baseUrl: "http://127.0.0.1:1", discoveries: [] };

    expect(renderStartupBanner(base)).toContain("   surrogates: opaque (FICTA_… tokens)\n");
    expect(renderStartupBanner({ ...base, surrogateStyle: "opaque" })).toContain(
      "   surrogates: opaque (FICTA_… tokens)\n",
    );
    expect(renderStartupBanner({ ...base, surrogateStyle: "typed" })).toContain(
      "   surrogates: typed (FICTA_<TYPE>_… tokens)\n",
    );
  });

  it("reports secret-shape detector posture from discovery", () => {
    const base = { protectedValues: 0, agentCommand: "claude", baseUrl: "http://127.0.0.1:1" };
    const active = [
      {
        id: "secret-shapes/detector",
        plugin: "secret-shapes",
        label: "Secret-shape detector",
        status: "active" as const,
      },
    ];

    expect(renderStartupBanner({ ...base, discoveries: active })).toContain(
      "   secret shapes: on (known token shapes, best-effort)\n",
    );
    expect(
      renderStartupBanner({
        ...base,
        discoveries: [
          { id: "secret-shapes/detector", plugin: "secret-shapes", label: "Secret-shape detector", status: "disabled" },
        ],
      }),
    ).toContain("   secret shapes: off\n");
  });

  it("suppresses launch diagnostics for default interactive TTY runs", () => {
  ...
  it("suppresses default diagnostics for machine-readable subprocess launches", () => {
    expect(shouldPrintStartupDiagnostics({ verbose: false, stderrIsTTY: false, machineReadable: true })).toBe(false);
    expect(shouldPrintStartupDiagnostics({ verbose: true, stderrIsTTY: false, machineReadable: true })).toBe(true);
  });
});
