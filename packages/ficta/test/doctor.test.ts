...
describe("ficta doctor", () => {
  ...
  it("reports the active surrogate token style, consistent with the startup banner", async () => {
    process.env.FICTA_REGISTRY_ENV_FILE_ENABLED = "0";
    process.env.FICTA_REGISTRY_PROCESS_ENV_ENABLED = "0";
    process.env.FICTA_REGISTRY_DOPPLER_ENABLED = "0";
    ...
    expect(report.config.surrogateStyle).toBe("opaque");
    expect(renderDoctorReport(report)).toContain("surrogate style: opaque (FICTA_… tokens)");
